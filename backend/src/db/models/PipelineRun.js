...
export const PipelineRun = {
  ...
  async updateStatus(id, status, extras = {}) {
    ...
    if (status === 'completed' || status === 'failed' || status === 'degraded') updates.finished_at = now();
    const db = getDB();
    if (db.type === 'supabase') {
  ...
  async incrementStep(id, field = 'steps_completed') {
    const db = getDB();
    if (db.type === 'supabase') {
      const { data: current } = await db.client.from(TABLE).select(field).eq('id', id).single();
      const { data, error } = await db.client
        .from(TABLE)
        .update({ [field]: (current?.[field] || 0) + 1, updated_at: now() })
        .eq('id', id)
        .select()
        .single();
      if (error) throw error;
  ...
  async getByStatus(status) {
    const db = getDB();
    if (db.type === 'supabase') {
      const { data, error } = await db.client.from(TABLE).select('*').eq('status', status);
      if (error) throw error;
      return data;
    }
